...
- 完善响应式布局机制和实时预览功能
- 新增媒体库集成和颜色选择器支持
- 新增8种组件类型，包括横幅大图、文本块、图片、按钮、容器、表单、卡片列表、分割线、文章列表
- 新增模板系统，支持默认模板、企业官网、科技公司、服务机构四种预设模板
- 新增主题系统，支持8种预设主题和暗夜模式
...
**更新** 本版本完全重构为基于React 19 + TypeScript的全新架构，采用现代化的Vite构建工具链，集成Ant Design 6.2.3组件库，支持TypeScript严格类型检查，提供完整的TypeScript类型定义和类型安全的组件系统。新增LOGO配置和页脚配置功能，增强组件渲染器的动态渲染能力，新增8种组件类型和模板系统，支持企业级内容管理的完整解决方案。
