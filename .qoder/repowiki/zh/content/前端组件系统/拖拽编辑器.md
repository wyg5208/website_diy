...
## 更新摘要
**变更内容**
- PageEditor组件进行了大规模清理，移除了包含25个不同图标映射的BUTTON_ICON_MAP常量对象，以及多个未使用的导入
- 将loading状态变量重命名为_loading以避免命名冲突，简化了组件复杂度
- 组件渲染器中的BUTTON_ICON_MAP常量对象仍然存在，用于按钮图标的渲染映射
- 按钮配置系统继续支持完整的图标选择功能，但BUTTON_ICON_MAP不再在PageEditor中重复定义
